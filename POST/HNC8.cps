...
description = "HNC8";
vendor = "HNC";
vendorUrl = "http://www.aengi.ge";
legal = "Copyright (C) 2022 by BM";
certificationLevel = 2;
minimumRevision = 45793;

longDescription = "Generic post for HNC8";
...
var retractOutput = createVariable({ prefix: "K", force: true }, peckFormat)
...
function onOpen() {
    ...
    writeBlock(gAbsIncModal.format(90),
        ...
        gFormat.format(40),
        gFormat.format(80));
    writeBlock("G109 B0") //Switch spindle to speed mode
...
function onSection() {
    ...
    if (insertToolCall || newWorkOffset || newWorkPlane || smoothing.cancel) {
        ...
        //BM: Moved retract to the end as it coused an error in controller "cant be in the same line" while doing 4-5 axis milling
    ...
    if (insertToolCall || operationNeedsSafeStart) {
        ...
        //BM: Added check for probe on toolchange

        // if (tool.type != TOOL_PROBE && currenttoolisprobe) {
        //     writeBlock(mFormat.format(106)); //Switch probe off
        //     writeBlock(mFormat.format(0));
        //     writeComment("Disconnect the probe");
        //     currenttoolisprobe = false;
        // }
        ...
        //BM: Added dwell to connect the probe
        if (tool.type == TOOL_PROBE) {
            writeBlock(mFormat.format(0), ";CONNECT THE PROBE");
            // writeComment("Connect the probe");
            writeBlock(mFormat.format(105)); //Switch probe on
            currenttoolisprobe = true;
        }
...
//Replaced (temporary) G65 P9810 with G31 G1
function protectedProbeMove(_cycle, x, y, z) {
    ...
    // writeBlock(gFormat.format(31), gFormat.format(1), _x, _y, _z, getFeed(cycle.feedrate))
}
        ...
        switch (cycleType) {
            ...
            case "right-tapping-with-chip-breaking":
                if (cycle.accumulatedDepth < cycle.depth) {
                    ...
                    //         getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
                    //         "P" + milliFormat.format(P),
                    //         peckOutput.format(cycle.incrementalDepth), retractOutput.format(cycle.chipBreakDistance),
                    //         pitchOutput.format(tool.threadPitch), "J2", 
                    //     );
                    //     forceFeed();
                    // } else {
                    ...
                    //         "P" + milliFormat.format(P),
                    //         peckOutput.format(cycle.incrementalDepth),
                    //         feedOutput.format(F), "J2"
                    //     );
                    // }

                    // BM. Changed this case: Added J2 (for B axis taping), forced fusion to use pitch for taping
                    // added E2/E1 switching (but E2 is not working, so we are waiting for our chinese friends).
                    Spindleinposmode = true;
                    var retrtocl = cycle.incrementalDepth < cycle.chipBreakDistance
                    if (retrtocl) {
                        error("There is still problem with G84 with E2 option, so chipBreakDistance must be less then or equal incrementalDepth")
                    }
                    writeBlock(
                        ...
                        "P" + milliFormat.format(P),
                        peckOutput.format(cycle.incrementalDepth),
                        retrtocl ? "" : retractOutput.format(cycle.chipBreakDistance),
            ...
            case "probing-x":
                ...
                // target = x + approach(cycle.approach1) * (cycle.probeClearance + tool.diameter / 2)
                // writeComment("x: " + x);
                // writeComment("cp.x: " + getCurrentPosition().x)
                // writeComment("t: " + target)
                // writeComment("ap: " + cycle.approach1)
                // if (x > getCurrentPosition().x) {
                //     ot = cycle.probeOvertravel + cycle.probeClearance
                // } else {
                //     ot = - (cycle.probeOvertravel + cycle.probeClearance)
                // }
                // ot = ot + cycle.probeClearance
                // ot = (cycle.probeOvertravel + cycle.probeClearance) * approach(cycle.approach1);
                // writeBlock(gFormat.format(1100), xOutput.format(ot));
                break;
            case "probing-y":
                protectedProbeMove(cycle, x, y, z - cycle.depth);
                writeBlock(
                ...
                // if (y > getCurrentPosition().y) {
                //     ot = cycle.probeOvertravel + cycle.probeClearance
                // } else {
                //     ot = -(cycle.probeOvertravel + cycle.probeClearance)
                // }
                // ot = ot + cycle.probeClearance
                // writeComment(getCurrentPosition().y, y)
                // ot = (cycle.probeOvertravel + cycle.probeClearance) * approach(cycle.approach1);

                // writeBlock(gFormat.format(1100), yOutput.format(ot));
                break;
            case "probing-z":
                safe_z = Math.min(z - cycle.depth + cycle.probeClearance, cycle.retract)
                protectedProbeMove(cycle, x, y, safe_z);
                protectedProbeMove(cycle, x, y, z - cycle.depth + cycle.probeClearance);
                writeBlock(
                    gFormat.format(65), "P" + 9811,
                ...
                // writeComment("cycle.depth: " + cycle.depth)
                // if (z > getCurrentPosition().z) {
                //     warning("Probing as possible only in Z- direction!")
                // } else {
                //     ot = -(cycle.probeOvertravel + safe_z - z + cycle.depth)
                // }
                // ot = ot + z - getCurrentPosition().z
                // ot = (cycle.probeOvertravel + safe_z) * approach(cycle.approach1);
                // // writeComment(ot);
                // writeBlock(gFormat.format(1100), zOutput.format(ot));
                break;
            case "probing-x-wall":
                protectedProbeMove(cycle, x, y, z);
                writeBlock(
...
function onCycleEnd() {
    if (isProbeOperation()) {
        ...
        // writeComment(cycle.feedrate)
        writeBlock(gFormat.format(31), gFormat.format(1), zOutput.format(cycle.retract), "F" + cycle.feedrate)
    } else {
        if (cycleSubprogramIsActive) {
            subprogramEnd();
            cycleSubprogramIsActive = false;
        ...
        //BM. Added M3 M5 for reseting STOC spindel mode after taping, which caused hanging
        if (Spindleinposmode) {
            // writeBlock("M03 M05");
            writeBlock("G109 B0")
            Spindleinposmode = false;
        }
    }
}
...
function onSectionEnd() {
    ...
    if (isProbeOperation()) {
        ...
        // writeBlock("M106") //Turn probe off
        if (getNextSection().getTool().type != TOOL_PROBE) {
            writeBlock(mFormat.format(106))
            writeBlock(mFormat.format(0), ";DISCONNECT THE PROBE")
        }
    ...
    if (words.length > 0) {
        switch (method) {
            case "G28":
                ...
                // writeBlock(gAbsIncModal.format(91), words);
                writeBlock(gAbsIncModal.format(90));
                break;
            case "G53":
                gMotionModal.reset();
